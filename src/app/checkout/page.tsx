...
export default function CheckoutPage() {
  ...
  const [chavePix, setChavePix] = useState("pedidofacil@pix.com.br");
  ...
  async function confirmar() {
    ...
    setCarregando(true);
    try {
      // Pega o clienteId do token JWT
      const tokenPayload = JSON.parse(atob(obterToken()!.split(".")[1]));
      const clienteId = tokenPayload.clienteId ?? 1;

      // 1. Cria o pedido
      const pedido = await api.criarPedido(clienteId);
      if (!pedido) throw new Error("Erro ao criar pedido.");

      // 2. Adiciona os itens
      for (const item of itens) {
        await api.adicionarItem(pedido.idPedido, item.produto.codProduto, item.quantidade);
      }

      // 3. Registra o pagamento
      const pagamento =
        metodo === "PIX"
          ? { tipo: "PIX", valor: total, chavePix }
          : {
              tipo: "CARTAO",
              valor: total,
              bandeira,
      ...
      await api.registrarPagamento(pedido.idPedido, pagamento);

      setConfirmado(pedido.idPedido);
      limpar();
    } catch (e: any) {
      setErro(e?.message ?? "Erro ao finalizar pedido. Tente novamente.");
    } finally {
      setCarregando(false);
    }
  }
        ...
        <main className="container" style={{ padding: "80px 24px", maxWidth: 560, textAlign: "center" }}>
          <div className="card rise" style={{ padding: 48 }}>
            ...
            <p className="muted" style={{ marginBottom: 8 }}>
              Status: <span className="badge badge-PAGO">pago</span>
            </p>
    ...
    <>
      ...
      >
        <section className="rise">
          ...
          {metodo === "PIX" ? (
            <div className="card rise" style={{ padding: 24, textAlign: "center" }}>
              <div
                ...
                style={{
                  ...
                  background: "repeating-conic-gradient(var(--ink) 0% 25%, var(--card) 0% 50%) 0 0 / 20px 20px",
                  borderRadius: 12, border: "6px solid var(--ink)",
                }}
              />
              <p style={{ fontWeight: 700 }}>Escaneie o QR Code</p>
              <p className="muted" style={{ fontSize: 13.5 }}>
                Chave: {chavePix} · expira em 30 min
              </p>
            </div>
          ) : (
            <div className="card rise" style={{ padding: 24 }}>
              <div className="field">
                ...
                <input value={numCartao} placeholder="0000 0000 0000 0000" onChange={(e) => setNumCartao(e.target.value)} />
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
                ...
                <div className="field">
                  ...
                  <select value={parcelas} disabled={tipoCartao === "DEBITO"} onChange={(e) => setParcelas(Number(e.target.value))}>
                    {[1, 2, 3, 4, 5, 6].map((n) => (
                      <option key={n} value={n}>{n}× de {formatBRL(total / n)}</option>
                    ))}
                  </select>
                </div>
              </div>
          ...
          {erro && <p style={{ color: "var(--error)", fontWeight: 600, marginTop: 14 }}>{erro}</p>}
        </section>
        ...
        <aside className="card rise d1" style={{ padding: 24, position: "sticky", top: 88 }}>
          ...
          <button
            ...
            style={{ width: "100%", marginTop: 20, opacity: carregando ? 0.7 : 1 }}
            onClick={confirmar}
            disabled={carregando}
          >
